from data_provider.data_factory_ecg import data_provider
...
class Exp_Long_Term_Forecast(Exp_Basic):
    ...
    def vali(self, vali_data, vali_loader, criterion, is_test=False):
        ...
        with torch.no_grad():
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(vali_loader):
                ...
                if self.args.model == 'ECG_Llama':
                    loss = criterion(outputs, batch_y)
                else:
                    loss = criterion(outputs, batch_y[:, -self.args.token_len:, :])
    ...
    def train(self, setting):
        ...
        for epoch in range(self.args.train_epochs):
            ...
            loss_val = torch.tensor(0., device=f"cuda:{self.device}")
            count = torch.tensor(0., device=f"cuda:{self.device}")
            ...
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(train_loader):
                ...
                if self.args.use_amp and self.args.model == 'ECG_Llama':
                    with torch.cuda.amp.autocast():
                        outputs = self.model(batch_x, batch_x_mark, None, batch_y_mark)
                        loss = criterion(outputs, batch_y)                        
                        loss_val += loss
            ...
            if self.args.model == 'ECG_Llama':
                ...
                vali_loss = self.vali(vali_data, vali_loader, criterion)
                test_loss = self.vali(vali_data, vali_loader, criterion, is_test=True)
            if (self.args.use_multi_gpu and self.args.local_rank == 0) or not self.args.use_multi_gpu:
